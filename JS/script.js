...
function applyFilterAndRender() {
  ...
  filteredArticles = allArticles.filter(
    (article) => typeFilter === "All" || article.type === typeFilter,
  );
  renderArticles();
...
function updateLoadMoreButton() {
  ...
  loadMoreBtn.style.display =
    filteredArticles.length === 0 ? "none" : "inline-block";
...
function updateSummaryCards(articles) {
  ...
  document.getElementById("countHigh").textContent = String(
    severityCounts.HIGH,
  );
  document.getElementById("countMedium").textContent = String(
...
function updateTypeChart(articles) {
  ...
  if (
    ...
    typeof window.typeChartInstance.destroy === "function"
  ) {
...
function updateSeverityChart(articles) {
  ...
  if (
    ...
    typeof window.severityChartInstance.destroy === "function"
  ) {
